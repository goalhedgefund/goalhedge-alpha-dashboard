...
describe('spread rule (R4: quoted spread = 3x statutory cost)', () => {
  ...
  it('QUOTE_ASK never sits below avgCost x (1 + minSpread)', () => {
    const e = engine();
...
describe('exits beyond the quote', () => {
  it('hard stop: mark <= avgCost x (1 - hardStopPct) sells at best bid, tagged', () => {
    const r = row(CE_ID, 'CE', 11_900, 12_000);
    const b = book('CE', 2, 15_000, NOW - 60_000, r);
    const out = engine().evaluate(baseInput({ books: [b] }));
    const exit = out.desired.find((d) => d.side === 'SELL');
    expect(exit!.reason).toBe('HARD_STOP');
    expect(exit!.limitPricePaise).toBe(11_900);
    expect(exit!.qty).toBe(2 * LOT);
  });

  it('theta ageing: a book older than maxHoldSec scratches at best bid', () => {
    const b = book('CE', 1, 15_000, NOW - 601_000);
    const out = engine().evaluate(baseInput({ books: [b] }));
    const exit = out.desired.find((d) => d.side === 'SELL');
    expect(exit!.reason).toBe('SCRATCH');
    expect(exit!.limitPricePaise).toBe(b.row!.bidPaise);
  });
  ...
  it('never short, and held + desired bid lots never exceed the cap', () => {
    fc.assert(
      fc.property(lotsArb, ({ ceLots, peLots, avgCost, mid }) => {
        ...
        // Every price is tick-snapped and positive.
